...
void do_echo_client(char* server_ip, char* file, char* my_ip, char* my_port);
void do_echo_server();
...
int main(int argc , char** argv) {
    ...
    if(strcmp(argv[1],"1") == 0) {
        ...
        do_echo_client(argv[2],NULL,argv[4],argv[5]);
    }
    return 0;
}
...
void do_continue_qeuest(int fd, sockaddr_in* server , socklen_t* len) {
	...
	count++;
        ...
        fprintf(stderr,"send Msg, len: %u, content: %d \n",msg.len, *(int*)(msg.data));
        timeval t;
        gettimeofday(&t, NULL);
...
void do_echo_client(char* server_ip, char* file, char* my_ip, char* my_port) {
    ...
    client.sin_port = htons(atoi(my_port));

    Inet_pton(AF_INET, server_ip, &server.sin_addr.s_addr);
    Inet_pton(AF_INET, my_ip, &client.sin_addr.s_addr);
